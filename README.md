...
```python
from meteaudata.types import ProcessingSteps, ProcessingType, FunctionInfo

def my_func(input_series: list[pd.Series], some_argument, some_keyword_argument=some_value) -> list[tuple[pd.Series, list[ProcessingSteps]]]:
    ...
    for series in input_series:
        transformed_series = series.apply(lambda x: x * 2)  # Example transformation: double the values
...
```python
from meteaudata.types import ProcessingStep, ProcessingType, FunctionInfo, Signal, DatasetTransformFunctionProtocol
import pandas as pd
import datetime
...
def my_dataset_func(
    ...
    *args, **kwargs,
    ...
    summed_time_series = TimeSeries(series=summed_series, processing_steps=[processing_step])
    new_signal = Signal(
        name=new_signal_name,
        units="some unit",
        provenance=final_provenance or input_signals[0].provenance,
        time_series={summed_time_series.name: summed_time_series}
    )
